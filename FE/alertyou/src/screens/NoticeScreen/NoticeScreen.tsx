...
const NoticeScreen = ({ navigation }: Props) => {
  ...
  const { data, refetch } = useQuery(['getNoticeList'], getNoticeList, {
    suspense: true,
    enabled: isFocused,
  });
  ...
  return (
    <View style={styles.container}>
      {data?.unRead.length !== 0 && (
        <TouchableOpacity activeOpacity={0.6} onPress={readAllNotice}>
          <Text style={styles.readContainer}>전체 읽기</Text>
        </TouchableOpacity>
      )}
      <Suspense>
        {data?.unRead.length !== 0 || data?.read.length !== 0 ? (
          ...
          <View
            style={{ flex: 1, justifyContent: 'center', marginBottom: 100 }}>
            <Center>
              <Image
                source={zeroNotice}
                style={{
