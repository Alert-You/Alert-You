...
export const getStudent = async (studentId: number | null | undefined): Promise<studentType> => {
  const result = await AxiosInstance.get(`teacher/student?studentId=${studentId}`)
    .then((response) => response);
  return result.data.student;
};

export const getReportList = async (studentId: number | null | undefined): Promise<repType[]> => {
  const result = await AxiosInstance.get(`report/list?userId=${studentId}`)
    .then((response) => response);
  return result.data.reports;
}

export const getProofList = async (studentId: number | null | undefined): Promise<proofType[]> => {
...
export const excludeStudent = async (studentId: number | null | undefined) => {
